"""
Drop-in SHADOW plugin template.

Copy this file, rename it (no leading underscore), fill in PLUGIN and run().
No other file needs to change — SHADOW discovers this automatically at startup.
"""
...
def run(parameters: dict, player=None, session_memory=None) -> str:
    ...
    player: the SHADOWUI instance — use player.write_log(f"SHADOW: ...") to log,
    ...
    if player:
        try:
            player.write_log(f"SHADOW: {result_text}")
        except Exception:
            pass
    return result_text
