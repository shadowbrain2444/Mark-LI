"""
SHADOW plugin — Pushup Counter (webcam vision).

Say "SHADOW, count my pushups" (any language) — the HUD switches to the
live camera, a 5-second countdown lets you get into position, then SHADOW
...
_FPS                 = 20
_CYAN   = (255, 190, 40)     # SHADOW cyan-blue (BGR)
_BRIGHT = (255, 235, 130)
...
def run(parameters: dict, player=None, session_memory=None) -> str:
    ...
    try:
        if stream_sig:
            ...
            view_open = True
        _log(f"SHADOW: Pushup session started — {mode}.")
        ...
        while True:
            ...
            if sig is not None and counter.update(sig):
                ...
                if counter.reps % 10 == 0 or (target and counter.reps == target):
                    _log(f"SHADOW: Pushup {counter.reps} 💪")
            if frame_sig:
                _emit_frame(frame_sig,
