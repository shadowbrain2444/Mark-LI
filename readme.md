...
MARK LI is the final form of the core: an assistant you extend without ever touching its engine. Drop a single plugin file into the `plugins/` folder and SHADOW learns a new skill on the next launch — no code changes, no configuration, no risk. On top of that, the voice itself got smarter: SHADOW now hears the emotion in your voice, knows when you're talking to someone else in the room and stays silent, and can hold one conversation for hours without losing the thread.
...
### 🧩 Plugin System — Extend SHADOW Without Touching a Single Core File
...
3. Restart SHADOW — done. The skill is live, by voice, in any language.
...
All three Live API upgrades degrade gracefully: if the preview API ever rejects them, SHADOW automatically reconnects with the standard configuration — users never see a crash.
